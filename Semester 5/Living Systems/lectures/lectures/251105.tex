...
if we focus on the case where \(a<0\) and \(p_{eq}^2=\frac{\lvert a \rvert}{b}\) then
...
\begin{equation}
	F_p = \frac{a}{2}\lvert\vec p\rvert^2+\frac{b}{4}\lvert\vec p\rvert^4
\end{equation}
...
How much free energy does it cost to create a continuous change in \(\theta \) in a system of dimension \(d\) with characteristic length \(L\)? From dimensional considerations \(\nabla^2 \sim L^{-2}\) and \(\int\dd \vec{r} \sim L^d\) therefore \(F\sim L^{d-2}\). For a large L and \(d\leq 2\) the cost is low. Therefore change in \(\theta \) will happen in a spontaneous manner from thermal fluctuations at \(T>0\).
...
\begin{equation}
	S = \sum\limits_i S_i(E_i-\frac{p_i^2}{2m_i})
\end{equation}

In a closed system the angular and linear momenta are conserved:

\begin{align}
	\sum\limits_i \vec{p_i} = const. && \sum\limits_i \vec{r_i} \times \vec{p_i} = const.
\end{align}

In thermal equilibrium the entropy is maximized while conforming to the constraints.

\begin{align}
	0 &= \diffp*{}{\vec{p_i}}\sum\limits_j \left[S_j\left(E_j-\frac{p_j^2}{2m_j}\right)+\vec a \cdot \vec p_j + \vec b \cdot \left(\vec r_i\times \vec p_j\right)\right]\\
	&= -\frac{1}{T}\frac{\vec p_i}{m_i}+\vec a + \vec b \times \vec r_i\\
	&\rightarrow \vec v_i = \vec u + \vec\Omega \times \vec r_i
\end{align}

where

\begin{align}
	\vec u = T \cdot \vec a && \vec \Omega = T \cdot \vec b
\end{align}
...
Assume a system with microstates \(\set{s} \) and a probability function \(P_s\) which changes with time. Its dynamics are given by a Master Equation:

\begin{equation}
	\diffp{}{t}P_s = \sum\limits_{s'}\left(P_{s'}W_{s'\rightarrow s} - P_s W_{s\rightarrow s'}\right)
\end{equation}
...
\begin{equation}
	P_{s'}W_{s'\rightarrow s} = P_s W_{s\rightarrow s'}
\end{equation}

Sometimes this is instead written as

\begin{equation}
	\frac{P_s}{P_{s'}} = \frac{W_{s'\rightarrow s}}{W_{s\rightarrow s'}}
\end{equation}