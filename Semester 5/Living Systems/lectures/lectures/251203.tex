...
\begin{description}
	...
	\item[\(\lvert\vec P\rvert \)] is a slow parameter near the critical points if \(P\) changes continuously in phase transition.
\end{description}
...
\end{equation}
where \(\tau\sim\sqrt{r}\), and in the thermodynamic limit \(r\) is large. We found \(\Delta\theta \) is insignificant for \(d>2\), and grows with the system's size for \(d<2\). The case \(d=2\) is marginal and gives \(\Delta\theta\sim l_n L\). This is similar to the calculation at equilibrium. It is important that the order is continuous because noise causes small fluctuations, in contrast to domain wall energy.