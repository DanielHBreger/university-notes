...
\begin{equation}
	\frac{2D}{\omega^2} = \frac{2k_BT}{\omega}\hat\chi''(\omega) = 2k_BT\frac{\mu}{\omega^2}
\end{equation}
implies
\begin{equation}
	\boxed{D = k_BT\mu}
...
We'll assume a set of fast variables $\{\psi_k\}$ and slow variables $\{\phi_i\}$ which satisfy equations of the form
\begin{align}
...
\begin{equation}
	G_k=0
\end{equation}
next we express $\{\psi_k\}$ in terms of $\{\phi_i\}$ and find
\begin{equation}
...
\begin{description}
	...
	and
	\begin{equation}
		\langle\xi_i(t)\xi_j(t')\rangle = 2k_BT\gamma\delta_{ij}\delta(t-t')
	\end{equation}
	...
	\begin{align}
		...
		&= -\gamma\int\ins{\partial_\alpha\frac{\delta F}{\delta\phi}}\ins{\partial_\alpha\frac{\delta F}{\delta\phi}}<0
	\end{align}
	...
	where
	\begin{equation}
		\dot F = \int\frac{\delta F}{\delta \phi}\diffp{\phi}{t}\dd r = -\gamma\int\ins{\frac{\delta F}{\delta \phi}}^2\dd r < 0
	\end{equation}
	so
	\begin{equation}
