...
\title{Select Topics in Living Systems}
\subtitle{Lectures}
\shorttitle{Living Systems} % For headers; if undefined, the usual title will be used
\ccode{01160611} % Most of these data are not compulsory
\subject{Subject of the Talk}
\author{Daniel Haim Breger}
\spemail{radar@technion.ac.il}
\email{danielbreger@campus.technion.ac.il}
\speaker{Dr. Ram Adar}
\date{29}{10}{2025}
\dateend{00}{02}{2026}
\conference{Physics 300}
\place{Technion}
...
\begin{figure}[!hbt]
  \includegraphics[width=1\linewidth]{polarvsisotropic}
...
The theory was developed by Lev Landau, for which he earned the Nobel Prize in Physics in 1962.

Close to the critical temperature $T_C$ the physics of phase transitions is universal. For a phase transition of 2nd order, we will write $F_\psi$ in a phenomenological form as a Taylor series expansion:
...
\subsubsection{Where a comes from}
...
\begin{figure}[!htb]
	\centering\includegraphics[width=1\linewidth]{phasetransition_arrows}
	\caption{A smooth phase transition with Domain Wall $\xi$.}
\end{figure}

\begin{figure}[!htb]
	\centering\includegraphics[width=1\linewidth]{transitionarea}
	\caption{A smooth phase transition with Domain Wall $\xi$.}
\end{figure}

We'll find a finite range around 0 of width $\xi$ in which $\abs{p}\neq p_{eq}$. This region is called the Domain Wall.

To quantify the cost to change $p$ in space we'll write the density of free energy per length unit:
