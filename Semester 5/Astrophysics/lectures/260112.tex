...
\begin{align}
  ...
  &= 4\pi^2 n_* R_*^2 \int\limits_0^\infty \dl r
\end{align}
...
\subsection{Measurements of Distances in the Universe}
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    \begin{groupplot}[
      ...
      % --- Left panel: Period--luminosity relation (schematic) ---
      \nextgroupplot[
        xlabel={$\tau$},
        ylabel={$L_*$},
        xmin=0, xmax=10,
      ...
      \addplot[only marks, mark=*, mark size=1.4pt] coordinates {
        (1.2,1.6) (1.8,2.2) (2.4,2.9) (3.0,3.4) (3.5,4.3)
        (4.2,4.7) (4.8,5.6) (5.2,6.1) (5.9,6.8) (6.4,7.4)
        (7.1,8.1) (7.7,8.7) (8.4,9.4)
      };
      % Best-fit line
      \addplot[red, very thick, domain=1:9] {0.2 + 1.08*x};

      % --- Right panel: periodic light curve (schematic) ---
      \nextgroupplot[
        xlabel={$t$},
        ylabel={$L_*$},
        xmin=0, xmax=10,
      ...
      \draw[<->, thick] (axis cs:2.2,9.0) -- (axis cs:7.2,9.0) node[midway, above] {$\tau$};
    \end{groupplot}
  \end{tikzpicture}
  \caption{Schematic Cepheid period--luminosity relation (left) and a periodic light curve (right), with characteristic period $\tau$. (Converted from a whiteboard drawing using OpenAI Prism)}
\end{figure}
...
\begin{equation}
  \frac{\lambda_{obs}}{\lambda_{em}} = 1+z = \sqrt{\frac{1+\beta}{1-\beta}} \simeq 1+ \beta
\end{equation}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[only marks, mark=*, mark size=1.3pt] coordinates {
                (0.8,1.0) (1.1,1.4) (1.4,1.2) (1.8,1.9) (2.0,2.4)
                (2.3,2.6) (2.7,2.9) (3.0,3.2) (3.3,3.6) (3.6,3.4)
                (4.0,4.1) (4.3,4.4) (4.7,4.9) (5.0,5.2) (5.4,5.5)
                (5.8,6.0) (6.1,6.2) (6.5,6.8) (6.9,7.0) (7.2,7.6)
                (7.6,7.9) (8.0,8.4)
            };

            % Best-fit line (schematic)
            \addplot[red, very thick, domain=0.6:9.2] {0.1 + x};

            % Markers on the distance axis (schematic)
            \draw[thick] (axis cs:5.0,0.15) -- (axis cs:5.0,-0.15) node[below] {$50$};
            \draw[thick] (axis cs:7.5,0.15) -- (axis cs:7.5,-0.15) node[below] {$100\,\mathrm{Mpc}$};
        \end{axis}
    \end{tikzpicture}
    \caption{Hubble-law plot (galaxy recession velocity vs. distance). This is after a correction made later because there was a problem with his original analysis; this is how we would interpret his measurements now. (Converted from a whiteboard drawing using OpenAI Prism)}
\end{figure}
...
\begin{equation}
  \dl s^2 = c^2 \dl t^2 - \dl x^2 -\dl y^2 - \dl z^2 \qquad x^\mu = \ins{t,x,y,z}
\end{equation}
