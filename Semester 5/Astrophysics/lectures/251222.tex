If our space does have a mass \(M\) at the position \(r=0\) then the metric for the space is the Schwarzschild metric (in spherical):
\begin{equation}
    g_{\mu\nu} = \begin{pmatrix}
        1-\frac{2GM}{rc^2}&&&\\
...
\begin{equation}
    ds^2 = \ins{1-\frac{R_s}{r}}\ins{cdt}^2 - \left[\ins{1-\frac{R_s}{r}}^{-1}dr^2+\dots\right]
\end{equation}
