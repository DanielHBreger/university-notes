...
from s3014a import envelope_decimate, Oscilloscope
...
def test_rk4_matches_exact_linear_circuit_and_time_grid():
    ...
    t,y=integrate([660],1,.000103,2e-7,20,g,t_skip=.0000201)
    Rt=R0+660
    ...
    assert t[0]>=.0000201
    assert t[0]==pytest.approx(.0000202)


def test_linear_extrapolation_outside_knots():
    g=make_g()
    assert g(50)!=g(40)
    assert float(g(51)-g(50))==pytest.approx(g.segments[-1][2])
    with pytest.raises(ValueError): make_g(shunt=216)


def test_decimation_preserves_real_simultaneous_samples_and_tail_spike():
    t=np.arange(1003,dtype=float); y=np.zeros((len(t),2))
    y[127,0]=11; y[239,1]=-9; y[-2,0]=99
    tt,yy=envelope_decimate(t,y,100)
    assert len(tt)<=100 and np.all(np.diff(tt)>0)
    np.testing.assert_array_equal(yy,y[tt.astype(int)])
    assert yy[:,0].max()==99 and yy[:,1].min()==-9
    assert tt[0]==0 and tt[-1]==1002


def test_instrument_mode_restored_after_failed_transfer():
    scope=Oscilloscope.__new__(Oscilloscope)
    writes=[]
    class Fake:
        def write(self,text): writes.append(text)
    scope.inst=Fake(); scope._prev_tb_mode='XY'
    def fail(*args): raise RuntimeError('transfer failed')
    scope._read_trace=fail
    with pytest.raises(RuntimeError): scope.get_trace()
    assert writes==[':TIMebase:MODE XY',':RUN']
...
def test_simulation_does_not_ingest_its_own_output_or_duplicate_overlays(tmp_path):
    from simulate import load_measured_bifurcation
    content='sweep,filename,rpot_ohm,max_v\nfwd,trace1.csv,600,2\nfwd,trace1.csv,600,2\n'
    (tmp_path/'sweep_bifurcation_points.csv').write_text(content)
    (tmp_path/'overlay_bifurcation_points.csv').write_text(content)
    (tmp_path/'simulated_bifurcation_points.csv').write_text('rpot_ohm,initial_v1_V,max_v\n600,1,3\n')
    assert load_measured_bifurcation(tmp_path)==[(600.,2.,'fwd'),(600.,2.,'fwd')]
